...
const handle = createAutomationHandler(CHAIN_NAME, MAILBOX, ROBOT, '0x', 'chainlink');
...
describe('automation handler', () => {
  ...
  test('gelato type forwards performData as-is and submits', () => {
    const gelatoHandle = createAutomationHandler(CHAIN_NAME, MAILBOX, ROBOT, '0x', 'gelato');
    const evmMock = EvmMock.testInstance(CHAIN_SELECTOR);
    evmMock.callContract = () => ({data: checkUpkeepResult(true, PERFORM_DATA)});
    evmMock.estimateGas = () => ({gas: 100_000n});
    evmMock.writeReport = () => ({txStatus: TxStatus.SUCCESS, txHash: hexToBytes(TX_HASH)});
    ...
    const runtime = newTestRuntime();
    expect(gelatoHandle(runtime as never)).toBe(TX_HASH);
  });
});

describe('createHandlers', () => {
  const network = (automations: {address: string; checkData: string; automationContractType: 'chainlink' | 'gelato'}[]) => ({
    schedule: '*/5 * * * *',
  ...
  test('creates one handler per automation', () => {
    const handlers = createHandlers(
      network([
        {address: ROBOT, checkData: '0x', automationContractType: 'chainlink'},
        {address: ROBOT, checkData: '0x', automationContractType: 'chainlink'},
      ]),
    );
  ...
  test('skips automations with empty address', () => {
    const handlers = createHandlers(
      network([{address: '', checkData: '0x', automationContractType: 'chainlink'}]),
    );
