import { bytesToHex, cre, encodeCallMsg, hexToBase64, TxStatus, type Runtime } from '@chainlink/cre-sdk';
...
export const processAutomation = (
  ...
  if (writeReportResult.txStatus !== TxStatus.SUCCESS) {
    runtime.log(
      `[${chainName}] writeReport status=${writeReportResult.txStatus} err=${writeReportResult.errorMessage ?? ''} for ${automationAddress} — skipping`
    );
    return null;
  }
  if (!writeReportResult.txHash) {
    runtime.log(`[${chainName}] writeReport returned SUCCESS but no txHash for ${automationAddress} — skipping`);
    return null;
  }
