...
export function encodeGsms(gsms: string[]): Hex {
  return encodeAbiParameters(parseAbiParameters('address[]'), [gsms as Hex[]]);
}
...
export const initWorkflow = (config: Config) => {
  ...
  return config.evms
    .filter((net) => net.chainName && net.receiver && net.gsms.length > 0)
