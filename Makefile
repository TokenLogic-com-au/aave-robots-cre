...
update :; forge update
install :; forge install && npm install && npm --prefix workflows/shared/offchain install && npm --prefix workflows/fee-shares-minter/offchain install && npm --prefix workflows/proof-of-reserve/offchain install
...
typecheck-proof-of-reserve :; cd workflows/proof-of-reserve/offchain && npm run typecheck
test-offchain-proof-of-reserve :; cd workflows/proof-of-reserve/offchain && npm test
...
deploy-proof-of-reserve :; @[ -n "$(DEPLOY_CHAIN_${env})" ] || { echo "ERROR: pass 'env=Avalanche'"; exit 1; }; \
	make deploy-account contract=workflows/proof-of-reserve/scripts/DeployProofOfReserveReceiver.s.sol:DeployProofOfReserveReceiver chain=$(DEPLOY_CHAIN_${env}) verifier="$(DEPLOY_VERIFIER_${env})" dry=${dry}
